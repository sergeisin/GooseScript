...
- В случае возникновения ошибки будет выведено соответствующее сообщение
...
- Изменение свойств ```Value``` или ```Quality``` приводит к мгновенной отправке сообщения и
...
Тип ```Value``` определяется исходя из значения параметра ```settings.mmsType```. 
В зависимости от типа данных для ```Value``` допустимы различные операции.
...
- Ссылки начинаются с имени IED, переданного в ```SaveSCL(iedName)``` первым аргументом
...
## Параметры Goose
Полный списко параметров ```GooseSettings``` приведён в таблице.
